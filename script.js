...
async function submitPost(selectedWardValue) { 
    ...
    if (imageFiles.length > 0) {
        ...
        base64Images.push('data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAASwAAACoCAMAAABpL3g+AAAAA1BMVEW+v78nB7LFAAAAJUlEQVR4nO3BAQ0AAADCoPdPbQ8H/wwAAAAAAAAAAAAAAAB+gABBAAECBL8nJgAAAABJRU5ErkJggg=='); // placeholder
    }
...
function setupDanhSachPage() {
    ...
    if (posts.length === 0) {
        container.innerHTML = '<p class="text-center text-gray-500 col-span-full">Chưa có tin nào được đăng.</p>';
    } else {
        posts.forEach((post, index) => {
            ...
            postCard.innerHTML = `
                <img src="${post.images[0]}" alt="${post.title}" class="post-thumbnail">
                <div class="post-content">
                    <h2 class="text-lg font-semibold text-gray-800 truncate">${post.title}</h2>
                    <p class="text-xl font-bold text-red-500 my-2">${Number(post.price).toLocaleString('vi-VN')} VNĐ</p>
                    <p class="text-gray-600 text-sm">${post.area} m² - ${post.ward || 'Cần Thơ'}</p>
...
function setupChiTietPage() {
    ...
    if (postId !== null && posts[postId]) {
        ...
        document.getElementById('detail-title').textContent = post.motelName || post.title; // Dùng tên trọ, nếu không có thì dùng tiêu đề
        
        document.getElementById('detail-price').textContent = `${Number(post.price).toLocaleString('vi-VN')} VNĐ`;
        ...
        document.getElementById('detail-contact-name').textContent = post.contactName;
       ...
       // }
        ...
        // ===============================================
        const imageDisplayContainer = document.getElementById('detail-images-display'); // Ảnh chính
        ...
        document.body.innerHTML = '<p class="text-center mt-20 text-xl font-bold">Không tìm thấy tin đăng này.</p>';
    }
}
...
function setupAdminPage() {
    ...
    if (sessionStorage.getItem('adminLoggedIn')) {
        ...
        loginForm.addEventListener('submit', (e) => {
            ...
            if (passwordInput.value === '123') { // Mật khẩu cứng
...
function loadPosts() {
    ...
    tableBody.innerHTML = '';
    if (posts.length === 0) {
        tableBody.innerHTML = '<tr><td colspan="5" class="text-center text-gray-500">Chưa có tin nào.</td></tr>';
    } else {
        posts.forEach((post, index) => {
            const row = document.createElement('tr');
            row.innerHTML = `
                <td>${index + 1}</td>
                <td><a href="chitiet.html?id=${index}" class="text-blue-600 hover:underline">${post.title}</a></td>
